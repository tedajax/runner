...
#define CONFIG_GET_AT_PROTO_NAMED(type, name) type config_get_##name##_at(Config* self, const char* key, u32 index)
#define CONFIG_TRY_GET_AT_PROTO_NAMED(type, name) type config_try_get_##name##_at(Config* self, const char* key, u32 index, type defaultValue)
...
#define CONFIG_GET_PROTO_NAMED(type, name) inline type config_get_##name##(Config* self, const char* key) { return CONFIG_GET_AT(name)(self, key, 0); }
#define CONFIG_TRY_GET_PROTO_NAMED(type, name) inline type config_try_get_##name##(Config* self, const char* key, type defaultValue) { return CONFIG_TRY_GET_AT(name)(self, key, 0, defaultValue); }
...
#define CONFIG_REGISTER_TYPE(type)  \
    CONFIG_GET_AT_PROTO(type);##CONFIG_TRY_GET_AT_PROTO(type);##CONFIG_GET(type);##CONFIG_TRY_GET(type)
